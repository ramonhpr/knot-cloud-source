...
function randomPassword(device) {
  // generate random password here
  var randomString = Math.random()                        // Generate random number, eg: 0.123456
                          .toString(36)                   // Convert  to base-36 : "0.4fzyo82mvyr"
                          .slice(-8);                     // Cut off last 8 characters : "yo82mvyr"
  device.user.password = randomString;
}

function encryptUser(device) {
  if (device.user) {
    device.token = device.user.token;
...
function setupHttpRoutes(server, skynet){
  ...
  server.post('/devices/user/reset', function (req, res) {
    skynet.sendActivity(getActivity('devices', req));
    req.merged_params.ipAddress = req.merged_params.ipAddress || getIP(req);
    if (!req.merged_params['user']['email']) {
      return res.status(500).json({ status: 401, message: 'Error: Unauthorized' });
      ...
      knotUser.getUserByEmail(req.merged_params['user']['email'], function (err, user) {
        if (err) {
          ...
          // send email to user
          knotMail.sendMail(device.user.email,device.user.password);
          // put on format of bcrypt
          device.user.password = bCrypt.hashSync(device.user.password, bCrypt.genSaltSync(10), null);
          // update gateways onlines
          getDevices(device, {owner: device.uuid}, false, function (error, getResData) {
            if(getResData.error){
            ...
            for (var i = 0; i < getResData.devices.length; i++) {
              var element = getResData.devices[i];
              console.log('element');
              console.log(element);
              if (element.online) {
                ...
                if (knotParentConnection) {
                    ...
                    console.log(knotParentConnection);
                }
              } else {
                // add to database notSync
              }
            }

          });
          // Update device
          updateFromClient(device, _.defaults({ uuid: device.uuid }, device), function (result) {
              var requestLog = { query: { uuid: device.uuid }, params: req.merged_params }
              if (result.error) {
                ...
                meshbluEventEmitter.emit('reset', { request: req.merged_params });
                res.status(201).json(device);
              }
          });
        } else {
          meshbluEventEmitter.emit('user-not-exists', { request: req.merged_params });
          return res.status(500).json({ status: 409, message:'Error: User not exists' });
        }
      });
    }
  });
